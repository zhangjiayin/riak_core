...
handle_call({service_up, Id, Pid}, _From, State) ->
    ...
    error_logger:info_msg("Marking service ~p up on pid ~p.\n", [Id, Pid]),
    S3 = local_update(S2),
...
handle_call({service_down, Id}, _From, State) ->
    ...
    error_logger:info_msg("Marking service ~p down by request.\n", [Id]),
    S3 = local_update(S2),
    {reply, ok, update_avsn(S3)};
...
handle_info({'DOWN', Mref, _, _Pid, _Info}, State) ->
    ...
    case erlang:get(Mref) of
        ...
        Id ->
            ...
            error_logger:info_msg("Marking service ~p down by exit ~p - ~p.\n", [Id, _Pid, _Info]),
            local_update(S2),
            {noreply, update_avsn(S2)}
    end;
