        ...
        false ->
            ...
            case ShouldHandoff of
                true ->
                    ...
                    case HOType of
                        repair ->
                            HOOpts = [{filter, Filter} | BaseOpts];
                        ownership_copy ->
                            %% TODO: this probably shouldn't be defined here
                            {ok, R0} = riak_core_ring_manager:get_raw_ring(),
                            R1 = riak_core_ring:future_ring(R0),
                            F = fun(K) ->
                                        Hashed = riak_core_util:chash_key(K),
                                        Pos0 = riak_core_ring:preflist_position(
                                                 Hashed, Src, R0),
                                        Pos1 = riak_core_ring:preflist_position(
                                                 Hashed, Target, R1),
                                        case {Pos0, Pos1} of
                                            {Same, Same} -> true;
                                            _ -> false
                            ...
                            HOOpts = [{filter, F} | BaseOpts];
                        _ ->
                            HOOpts = [{filter, none} | BaseOpts]
                    end,
